...
class BidirectionalTransformer(nn.Module):
    def __init__(self, configs):
        ...
        self.pos_emb = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(configs["num_image_tokens"], configs["dim"])),
            0.0,
            0.02,
        )
        ...
        self.blocks = nn.Sequential(
            *[
                Encoder(configs["dim"], configs["hidden_dim"])
        ...
        self.bias = nn.Parameter(
            torch.zeros(self.num_image_tokens, configs["num_codebook_vectors"] + 1)
        )
